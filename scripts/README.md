...
**重要な注意**: website差し替え候補はURLのスラッグと地名を照合して機械的に検証できる
（例: `cinema.aeoncinema.com/wm/kuwana/` の「kuwana」が「桑名」と一致するか）が、座標の
差し替え候補には同種の検証手段がない。`nearestByName`は「名前一致した候補の中で一番近い」
ものを選ぶだけなので、ブランド名のみのOSMタグが閉館済み店舗と現役店舗の両方に部分一致する
場合、たまたま近い方＝正しい方とは限らない（三重県のイオンシネマ津南で実際に発生：
「イオンシネマ」という汎用タグが閉館済み「イオンシネマ津」の座標を指しており、機械的に
採用すると誤った場所に移動してしまった）。座標の差し替え候補は住所とWebSearchで
必ず裏を取ってから適用する。
